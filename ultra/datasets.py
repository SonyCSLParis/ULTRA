...
class NarrativeInductiveDataset(InductiveDataset):
    ...
    # @property
    # def processed_vocab(self):
    #     return os.path.join(self.root, self.name, self.version, "vocab")
    ...
    def process(self):
        ...
        vocab = {
            "train": train_res, "inference": inference_res, "valid": valid_res, "test": test_res
        }
        ...
        torch.save((self.collate([train_data, valid_data, test_data])), self.processed_paths[0])
        vocab_path = os.path.join(self.root, self.name, self.version, "vocab")
